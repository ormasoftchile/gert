...
export class GertClient {
  ...
  async start(): Promise<void> {
    ...
    // Ensure the gert binary dir is on PATH so child commands can find `gert`
    // On Windows, process.env is a case-insensitive proxy but spreading it creates
    // a plain object. The actual key is typically "Path" not "PATH". We must find
    // the real key to avoid creating a duplicate that shadows the system paths.
    const env = { ...process.env };
    const pathKey = Object.keys(env).find(k => k.toLowerCase() === 'path') || 'PATH';
    const pathSep = process.platform === 'win32' ? ';' : ':';
    const currentPath = env[pathKey] || '';
    if (!currentPath.includes(gertDir)) {
      env[pathKey] = gertDir + pathSep + currentPath;
    }
