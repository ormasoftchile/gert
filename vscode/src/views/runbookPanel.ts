...
export class RunbookPanel {
  ...
  private icmId: string | null = null; // ICM incident ID — persists across chain transitions
  ...
  static async create(
    ...
    options?: { icmId?: string; scenarioDir?: string; sourceFile?: string; tsgName?: string; sourceMapping?: Record<string, { start: number; end: number }>; inputDefs?: Record<string, any>; chainHistory?: ChainEntry[] }
    ...
    // Build tab title: ICM ID (if available) + truncated TSG name
    const shortName = displayName.length > 25 ? displayName.substring(0, 25) + '…' : displayName;
    const icmPrefix = options?.icmId ? `ICM-${options.icmId} | ` :
                      options?.scenarioDir?.match(/icm-(\d+)/) ? `ICM-${options.scenarioDir.match(/icm-(\d+)/)![1]} | ` : '';
    const tabTitle = `${icmPrefix}${shortName}`;
    ...
    try {
      ...
      rbPanel.gertCwd = require('path').dirname(require('path').dirname(client.getGertPath())); // gert serve CWD = gertRoot
      ...
      // Persist ICM ID across chain transitions
      rbPanel.icmId = options?.icmId || (options?.scenarioDir?.match(/icm-(\d+)/)?.[1]) || null;
    ...
    switch (msg.method) {
      ...
      case 'event/stepCompleted':
        ...
        if (this.autoRunning) {
          ...
          // Interactive mode: auto-advance XTS/CLI steps that passed
    ...
    switch (msg.type) {
      ...
      case 'chainToRunbook':
        try {
          ...
          // Update panel title — keep ICM prefix if present
          const currentTitle = this.panel.title;
          const icmMatch = currentTitle.match(/^ICM-\d+ \| /);
          const childShort = childName.length > 25 ? childName.substring(0, 25) + '…' : childName;
          this.panel.title = icmMatch ? `${icmMatch[0]}${childShort}` : childShort;
      ...
      case 'copySummary': {
        if (msg.text) {
          vscode.env.clipboard.writeText(msg.text).then(() => {
            vscode.window.showInformationMessage('Gert: Summary copied to clipboard — paste into ICM discussion, email, or Teams.');
          });
        }
        break;
      }
      ...
      case 'openIcm': {
        if (msg.icmId) {
          ...
          const pattern = config.get<string>('icmLinkPattern', 'https://portal.microsofticm.com/imp/v3/incidents/details/{id}/home');
          const url = pattern.replace('{id}', msg.icmId);
...
<style>
  ...
  .workflow-header .badge.icm { background: #264f78; color: #9cdcfe; font-weight: normal; }
  ...
  .type-badge.xts { background: #264f78; color: #9cdcfe; }
...
<div class="layout">
  ...
  <div class="workflow-map" id="workflowMap" style="width:${this.mapWidth}">
    <div class="workflow-header">
      ...
      ${this.getIcmId() ? this.renderIcmBadge() : ''}
...
<script>
  ...
  function openIcm(id) { vscode.postMessage({ type: 'openIcm', icmId: id }); }
  ...
  /**
   * Build a structured text summary for sharing (ICM note, email, Teams, etc.)
   */
  private buildSummaryText(): string {
    ...
    lines.push(`| | |`);
    lines.push(`|---|---|`);
    const icmId = this.getIcmId();
    if (icmId) {
      lines.push(`| **ICM** | ${icmId} |`);
    }
  ...
  /** Extract ICM ID — uses persistent field set at panel creation. */
  private getIcmId(): string | null {
    return this.icmId;
  }

  /** Render the ICM badge — clickable link or plain text based on settings. */
  private renderIcmBadge(): string {
    const id = this.getIcmId();
    if (!id) return '';
    const config = vscode.workspace.getConfiguration('gert');
    const linkEnabled = config.get<boolean>('icmLinkEnabled', true);
    if (linkEnabled) {
      return `<span class="badge icm clickable" title="Open ICM ${id}" onclick="openIcm('${id}')" style="cursor:pointer;text-decoration:underline">ICM ${id}</span>`;
    }
    return `<span class="badge icm" title="ICM Incident">ICM ${id}</span>`;
  }
  ...
  private classifyStepsForProse(nodes: any[], triage: any[], mitigation: any[], depth = 3) {
    for (const node of nodes) {
      ...
      // XTS queries and checks go to triage; manual routing/actions go to mitigation
      if (step.type === 'xts' || (step.type === 'manual' && !step.outcomes?.some((o: any) => o.next_runbook))) {
  ...
  private renderStepAsProse(node: any, states: Map<string, string>, headingLevel: number): string {
    ...
    // Query as code block — check both step.xts.query (raw) and step.query (from server)
    const query = step.query || step.xts?.query;
    if (query) {
