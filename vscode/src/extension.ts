...
export function activate(context: vscode.ExtensionContext) {
  ...
  function resolveGertPath(): string {
    ...
    if (!gertPath || gertPath === 'gert') {
      ...
      const ext = isWin ? '.exe' : '';
      const candidates = [
        // Workspace root (built with `go build -o gert ./cmd/gert/`)
        path.join(vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? '', `gert${ext}`),
        // bin/ subdirectories relative to extension
        path.join(context.extensionPath, '..', '..', 'bin', `gert${ext}`),
        path.join(context.extensionPath, '..', 'bin', `gert${ext}`),
      ];
  ...
  const runTsgCommand = vscode.commands.registerCommand(
    ...
    async (uri?: vscode.Uri) => {
      ...
      const mode = (modePick.label === 'real (saved inputs)') ? 'real' : modePick.label;
