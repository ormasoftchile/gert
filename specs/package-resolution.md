...
require:                            # optional: external package dependencies
  ...
  gert-xts: ../gert-xts            # name → local path
  # gert-xts: registry:gert/xts@v2 # future: registry URI (not implemented)
...
config:                             # optional: runtime configuration (replaces .gert/config.yaml)
  providers:
    icm:
...
- `config` holds runtime settings (providers, environment). Replaces the former `.gert/config.yaml` — everything in one file.
...
tools:
  ...
  - gert-xts/xts                # package: gert-xts/tools/xts.tool.yaml
...
Has `tools/curl.tool.yaml`, `tools/nslookup.tool.yaml`, `tools/ping.tool.yaml`. No runbooks to reference externally. Runtime provider config lives in the `config:` section.

### `gert-xts` (xts provider + tool)
...
Has `tools/xts.tool.yaml`. Runbooks that need XTS reference `gert-xts/xts`.
...
| `cmd/gert/main.go` | Walk up from runbook to find `gert.yaml`; build project context; load `config` section for providers |
