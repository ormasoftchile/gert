...
type ScenarioInfo struct {
	Name    string // directory name (e.g. "scenario-748724360")
...
func (r *Runner) executeReplay(runbookPath string, originalRB *schema.Runbook, scenario ScenarioInfo) (*RunResult, error) {
	...
	if stepScenario.Scenario != nil && len(stepScenario.Scenario.Evidence) > 0 {
		collector = providers.NewScenarioCollector(stepScenario.Scenario.Evidence)
	} else {
