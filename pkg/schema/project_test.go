...
func TestResolveToolRefUnqualifiedFromRequire(t *testing.T) {
	// Main project has no local tools, but requires a package that exports "query"
	mainRoot := t.TempDir()
	depRoot := t.TempDir()

	// Create dep package with query tool and exports
	depTools := filepath.Join(depRoot, "tools")
	os.MkdirAll(depTools, 0755)
	os.WriteFile(filepath.Join(depTools, "query.tool.yaml"), []byte("apiVersion: tool/v0\n"), 0644)
...
`), 0644)
	...
	resolved, err := proj.ResolveToolRef("query")
	if err != nil {
		t.Fatalf("ResolveToolRef unqualified from require: %v", err)
	}
	expected := filepath.Join(depRoot, "tools", "query.tool.yaml")
	if resolved != expected {
		t.Fatalf("resolved=%q want %q", resolved, expected)
	}
}
...
func TestResolveToolRefQualified(t *testing.T) {
	...
	resolved, err := proj.ResolveToolRef("dep-pkg/query")
	if err != nil {
		t.Fatalf("ResolveToolRef: %v", err)
	}
	expected := filepath.Join(depRoot, "tools", "query.tool.yaml")
...
func TestSplitRef(t *testing.T) {
	tests := []struct {
		...
		{"dep-pkg/query", "dep-pkg", "query", true},
...
`), 0644)
	...
	svc, ok := proj.Config.Providers["svc"]
	if !ok {
		t.Fatal("expected svc provider")
	}
	if svc.Binary != "gert-svc-provider" {
		t.Fatalf("svc.Binary=%q want gert-svc-provider", svc.Binary)
	}
}
...
`), 0644)
	...
	if proj.Exports.Tools["query"] != "providers/query" {
		t.Fatalf("exports.tools.query=%q", proj.Exports.Tools["query"])
	}
}
