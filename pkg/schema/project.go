...
//
// Qualified name ("dep-pkg/query"):
//
//	→ look up "dep-pkg" in require → resolve package → <pkg>/tools/query.tool.yaml
...
func (p *Project) ResolveToolRef(ref string) (string, error) {
	...
	if !qualified {
		// Unqualified: local tool first
		candidate := filepath.Join(p.Root, p.ToolsDir(), name+".tool.yaml")
		if _, err := os.Stat(candidate); err == nil {
			return candidate, nil
		}
		// Fallback: search required packages' exports for this tool name
		for pkgName := range p.Require {
			...
			// Also try direct path in the required package
			candidate = filepath.Join(pkg.Root, pkg.ToolsDir(), name+".tool.yaml")
			if _, err := os.Stat(candidate); err == nil {
				return candidate, nil
			}
		}
		return "", fmt.Errorf("tool %q not found in package %q or any required packages",
			name, p.Name)
	}
...
//   - name: tool reference (e.g. "nslookup" or "dep-pkg/query")
