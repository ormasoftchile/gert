...
func ValidateDomain(rb *Runbook) []*ValidationError {
	...
	// Validate tree nodes (step, iterate, invoke, tool)
	if len(rb.Tree) > 0 {
		...
		walkTree = func(nodes []TreeNode, path string) {
			for i, n := range nodes {
				...
				// Validate iterate blocks
				if n.Iterate != nil {
					iterPath := nodePath + ".iterate"
					if n.Step.ID != "" {
						errs = append(errs, &ValidationError{
							Phase:    "domain",
							Path:     nodePath,
							Message:  "tree node cannot have both 'step' and 'iterate'",
							Severity: "error",
						})
					}
					if len(n.Branches) > 0 {
						errs = append(errs, &ValidationError{
							Phase:    "domain",
							Path:     nodePath,
							Message:  "iterate node cannot have 'branches' (use branches inside iterate steps instead)",
							Severity: "error",
						})
					}

					isListMode := strings.TrimSpace(n.Iterate.Over) != ""
					hasUntil := strings.TrimSpace(n.Iterate.Until) != ""

					if isListMode {
						// List mode: over is set — max and until must NOT be set
						if n.Iterate.Max != 0 {
							errs = append(errs, &ValidationError{
								Phase:    "domain",
								Path:     iterPath + ".max",
								Message:  "iterate with 'over' must not set 'max' (list length determines iterations)",
								Severity: "error",
							})
						}
						if hasUntil {
							errs = append(errs, &ValidationError{
								Phase:    "domain",
								Path:     iterPath + ".until",
								Message:  "iterate with 'over' must not set 'until' (list mode and convergence mode are mutually exclusive)",
								Severity: "error",
							})
						}
					} else {
						// Convergence mode: max and until are required
						if n.Iterate.Max < 1 {
							errs = append(errs, &ValidationError{
								Phase:    "domain",
								Path:     iterPath + ".max",
								Message:  "iterate max must be at least 1",
								Severity: "error",
							})
						}
						if !hasUntil {
							errs = append(errs, &ValidationError{
								Phase:    "domain",
								Path:     iterPath + ".until",
								Message:  "iterate requires an 'until' convergence condition",
								Severity: "error",
							})
						}
					}

					// 'as' without 'over' is an error
					if strings.TrimSpace(n.Iterate.As) != "" && !isListMode {
						errs = append(errs, &ValidationError{
							Phase:    "domain",
							Path:     iterPath + ".as",
							Message:  "iterate 'as' requires 'over' (as names the loop variable in list mode)",
							Severity: "error",
						})
					}

					if len(n.Iterate.Steps) == 0 {
						errs = append(errs, &ValidationError{
							Phase:    "domain",
							Path:     iterPath + ".steps",
							Message:  "iterate requires at least one step",
							Severity: "error",
						})
					}
					// Recurse into iterate steps
					walkTree(n.Iterate.Steps, iterPath+".steps")
					continue
				}

				// Non-iterate node must have a step
				if n.Step.ID == "" {
					errs = append(errs, &ValidationError{
						Phase:    "domain",
						Path:     nodePath,
						Message:  "tree node requires either 'step' or 'iterate'",
						Severity: "error",
					})
					continue
				}
...
func validateToolStepsDeep(rb *Runbook, toolDefs map[string]*ToolDefinition) []*ValidationError {
	...
	walkTree = func(nodes []TreeNode, path string) {
		for i, n := range nodes {
			nodePath := fmt.Sprintf("%s[%d]", path, i)
			if n.Iterate != nil {
				walkTree(n.Iterate.Steps, nodePath+".iterate.steps")
				continue
			}
