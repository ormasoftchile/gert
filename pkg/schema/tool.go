...
// transport, actions, typed arguments, capture rules, and governance constraints.
type ToolDefinition struct {
	...
	Actions    map[string]ToolAction `yaml:"actions"     json:"actions"     jsonschema:"required"`
}
...
func ValidateToolDefinition(td *ToolDefinition) []*ValidationError {
	...
	if len(td.Actions) == 0 {
		errs = append(errs, &ValidationError{
			Phase:    "domain",
			Path:     "actions",
			Message:  "tool definition requires at least one action",
			Severity: "error",
		})
	}
