...
callSucceeded:
	...
	return &ActionResult{
		...
		Captures: captures,
		Duration: duration,
		Usage:    parseUsageFromJSON(resultRaw),
	}, nil
}
...
// parseUsageFromCaptures checks if the captures contain a "_usage" key with
// a JSON-encoded UsageReport and parses it out. If present, the "_usage" key
// is removed from captures (it's metadata, not a user capture).
func parseUsageFromCaptures(captures map[string]string) *providers.UsageReport {
	raw, ok := captures["_usage"]
	if !ok || raw == "" {
		return nil
	}
	var usage providers.UsageReport
	if err := json.Unmarshal([]byte(raw), &usage); err != nil {
		return nil
	}
	delete(captures, "_usage")
	return &usage
}

// parseUsageFromJSON tries to extract a "_usage" object from raw JSON-RPC result bytes.
func parseUsageFromJSON(raw json.RawMessage) *providers.UsageReport {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(raw, &obj); err != nil {
		return nil
	}
	usageRaw, ok := obj["_usage"]
	if !ok {
		return nil
	}
	var usage providers.UsageReport
