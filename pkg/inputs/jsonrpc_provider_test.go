...
func TestJSONRPCInputProviderIntegration(t *testing.T) {
	...
	def := &schema.ProviderDefinition{
		...
		Meta:       schema.ProviderMeta{Name: "mock", Binary: mockBin},
...
// TestLoadProviderFile tests loading the ICM provider definition.
func TestLoadProviderFile(t *testing.T) {
	pd, err := schema.LoadProviderFile("../../configs/icm.provider.yaml")
	if err != nil {
		t.Fatalf("LoadProviderFile: %v", err)
	}
	if pd.APIVersion != "provider/v0" {
		t.Errorf("apiVersion = %q", pd.APIVersion)
	}
	if pd.Meta.Name != "icm" {
		t.Errorf("name = %q", pd.Meta.Name)
	}
	if pd.Meta.Binary != "gert-icm-provider" {
		t.Errorf("binary = %q", pd.Meta.Binary)
	}
	if pd.Capabilities.ResolveInputs == nil || len(pd.Capabilities.ResolveInputs.Prefixes) == 0 {
		t.Error("expected resolve_inputs capability with prefixes")
	}
}
