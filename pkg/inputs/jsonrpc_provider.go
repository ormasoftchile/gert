...
// NewJSONRPCInputProvider creates a provider from a tool definition that
// declares resolve_inputs capabilities. The process is spawned lazily on first Resolve call.
func NewJSONRPCInputProvider(def *schema.ToolDefinition) *JSONRPCInputProvider {
...
func (p *JSONRPCInputProvider) Prefixes() []string {
	if p.def.Capabilities != nil && p.def.Capabilities.ResolveInputs != nil {
...
func spawnProvider(ctx context.Context, binary string, argv []string, startup *schema.ToolStartup) (*providerProcess, error) {
	// If the binary is a bare name (no path separator), try to find it next
	// to the current executable before falling back to PATH lookup.
	resolvedBinary := binary
	if !filepath.IsAbs(binary) && !strings.ContainsAny(binary, `/\`) {
		if selfExe, err := os.Executable(); err == nil {
			candidate := filepath.Join(filepath.Dir(selfExe), binary)
			// On Windows, also try with .exe suffix
			if _, err := os.Stat(candidate); err == nil {
				resolvedBinary = candidate
			} else if _, err := os.Stat(candidate + ".exe"); err == nil {
				resolvedBinary = candidate + ".exe"
			}
		}
	}

	cmd := exec.CommandContext(ctx, resolvedBinary, argv...)
	cmd.Env = os.Environ()
