...
func TestManagerResolve_SingleProvider(t *testing.T) {
	...
	mgr.Register(&mockProvider{
		prefixes: []string{"icm."},
		resolved: map[string]string{
			"icm.customFields.ServerName": "sql-server-01",
			"icm.severity":               "2",
		},
	})

	inputs := map[string]*schema.InputDef{
		"server_name": {From: "icm.customFields.ServerName"},
...
func TestManagerResolve_MultipleProviders(t *testing.T) {
	...
	mgr.Register(&mockProvider{
		prefixes: []string{"icm."},
		resolved: map[string]string{"icm.severity": "3"},
	})
	mgr.Register(&mockProvider{
		prefixes: []string{"pd."},
...
func TestBindingsFromInputs(t *testing.T) {
	inputs := map[string]*schema.InputDef{
		"server": {From: "icm.customFields.ServerName", Pattern: ".*"},
		"sev":    {From: "icm.severity"},
		"host":   {From: "prompt"},
		"region": {From: "pd.region"},
	}

	bindings := BindingsFromInputs(inputs, "icm.")
	if len(bindings) != 2 {
		t.Errorf("expected 2 icm bindings, got %d", len(bindings))
	}
	if bindings["server"].From != "icm.customFields.ServerName" {
