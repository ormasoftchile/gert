...
type ResolveRequest struct {
	...
	Context  map[string]string       // execution context (e.g. {"icmId": "12345"})
}

// InputBinding describes a single input's source binding.
type InputBinding struct {
	From    string // e.g. "icm.customFields.ServerName"
...
// Implementations include the built-in ICM provider and future external
// providers (PagerDuty, ServiceNow, etc.).
type InputProvider interface {
	// Prefixes returns the `from:` prefixes this provider handles (e.g. ["icm."]).
	Prefixes() []string
