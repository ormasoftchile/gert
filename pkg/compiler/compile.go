...
func CompileTSGFromSource(source []byte, sourceName string, client LLMClient) (*CompileResult, error) {
	...
	// Count code blocks and decision points from IR to set minimum step target
	minSteps := 0
...
func WriteRunbook(rb *schema.Runbook, path string) error {
	...
	if existing, err := os.ReadFile(path); err == nil {
		...
		if err := yaml.Unmarshal(existing, &old); err == nil {
			...
			if oldSteps > 0 && newSteps < oldSteps {
